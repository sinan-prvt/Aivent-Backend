...
USER_SERVICE_URL = os.getenv("USER_SERVICE_URL", "http://user_service:8001/api/internal/users/")
USER_SERVICE_INTERNAL_TOKEN = os.getenv("USER_SERVICE_INTERNAL_TOKEN", None)
...
class VerifyVendorOTPView(APIView):
    ...
    def post(self, request):
        ...
        if USER_SERVICE_INTERNAL_TOKEN:
            headers["Authorization"] = f"Bearer {USER_SERVICE_INTERNAL_TOKEN}"

        try:
            resp = requests.post(USER_SERVICE_URL.rstrip("/") + "/", json=payload, headers=headers, timeout=5)
...
class VendorApproveView(APIView):
    ...
    def post(self, request, vendor_id):
        ...
        if not vendor.user_id:
            return Response({"detail":"Vendor has no linked user"}, status=400)

        vendor.status = "approved"
        vendor.save()

        url = f"{USER_SERVICE_URL.rstrip('/')}/{vendor.user_id}/approve-vendor/"
        headers = {}
        if USER_SERVICE_INTERNAL_TOKEN:
            headers["Authorization"] = f"Bearer {USER_SERVICE_INTERNAL_TOKEN}"
        ...
        try:
            ...
            return Response({"detail":"failed to update user-service", "error": str(e)}, status=502)

        return Response({"detail":"Vendor approved and user-service updated"}, status=200)
